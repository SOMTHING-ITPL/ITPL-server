...
func NewCourseHandler(db *gorm.DB, userRepo *user.Repository, pRepo *performance.Repository, bucketBasics *aws_client.BucketBasics) *CourseHandler {
...
func (h *CourseHandler) GetCourseDetails() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		var imageURL *string
		if co.ImageKey != nil {
			URL, err := aws_client.GetPresignURL(h.bucketBasics.AwsConfig, h.bucketBasics.BucketName, *co.ImageKey)
			if err != nil {
	...
	return func(c *gin.Context) {
		...
		for _, course := range courses {
			...
			if course.ImageKey != nil {
				URL, err := aws_client.GetPresignURL(h.bucketBasics.AwsConfig, h.bucketBasics.BucketName, *course.ImageKey)
				if err != nil {
					c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to get image URL"})
					return
				}
...
func (h *CourseHandler) ModifyCourseImage() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		key, err := aws_client.UploadToS3(h.bucketBasics.S3Client, h.bucketBasics.BucketName, fmt.Sprintf("course_images/%d", ucourseID) /*prefix*/, file)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to upload image"})
			return
		}
