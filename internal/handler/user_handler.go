...
func NewUserHandler(userRepository *user.Repository, smtpRepository *email.Repository, awsBucket *aws_client.BucketBasics) *UserHandler {
...
func (h *UserHandler) GetUser() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		var photoURL *string
		if user.Photo != nil {
			url, err := aws_client.GetPresignURL(h.BucketBasics.AwsConfig, h.BucketBasics.BucketName, *user.Photo)
			if err != nil {
...
func (h *UserHandler) UpdateProfile() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if err == nil {
			if user.Photo != nil {
				err = aws_client.DeleteImage(h.BucketBasics.S3Client, h.BucketBasics.BucketName, *user.Photo)
				if err != nil {
			...
			url, err := aws_client.UploadToS3(h.BucketBasics.S3Client, h.BucketBasics.BucketName, fmt.Sprintf("profile/%d", userID), file)
			if err != nil {
		...
		if updatedUser.Photo != nil {
			url, err := aws_client.GetPresignURL(h.BucketBasics.AwsConfig, h.BucketBasics.BucketName, *updatedUser.Photo)
			if err != nil {
				c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to get photo in aws: " + err.Error()})
				return
			}
...
func (h *UserHandler) GetGenres() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		for _, g := range genres {
			url, err := aws_client.GetPresignURL(h.BucketBasics.AwsConfig, h.BucketBasics.BucketName, g.ImageKey)
			if err != nil {
...
func (h *UserHandler) PutGenre() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		url, err := aws_client.UploadToS3(h.BucketBasics.S3Client, h.BucketBasics.BucketName, fmt.Sprintf("genre/%s", name), file)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
